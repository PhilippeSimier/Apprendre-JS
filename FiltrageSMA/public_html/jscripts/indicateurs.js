			...
			for (var type in indicators) {
				...
				if (ind.values !== UNDEFINED && ind.visible !== false && ((ind.options && ind.options.params) && ind.options.params.allowTooltip !== false)) {
					...
					each(ind.values, function (val, k) {
						if (isNaN(val.y)) { // #31
							indicatorTooltip += '<span style="color:' + val.color + ';">' + '● </span>';
                                                        indicatorTooltip += splat(ind.options.names || ind.name)[k];
                                                        indicatorTooltip += ': <span style="font-weight:bold;">' + HC.numberFormat(val.y, 2) + '</span><br/>';
						}
					});
				}
			}
